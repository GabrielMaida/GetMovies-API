...
BEGIN;
    ...
    IF @codigo_lista = 1
    BEGIN;
        SELECT * FROM filme WHERE data_lancamento > GETDATE() ORDER BY data_lancamento ASC;
    END;

    -- Codigo da lista igual a 2: Listar filmes lançados em 2024 até a data atual
    ELSE IF @codigo_lista = 2
    BEGIN;
        SELECT * FROM filme WHERE data_lancamento BETWEEN '2024-01-01' AND GETDATE() ORDER BY data_lancamento ASC;
    END;

    -- Codigo da lista igual a 3: Listar filmes lançados em 2023
    ELSE IF @codigo_lista = 3
    BEGIN;
        SELECT * FROM filme WHERE data_lancamento BETWEEN '2023-01-01' AND '2023-12-31' ORDER BY data_lancamento ASC;
    END;

    -- Codigo da lista igual a 4: Listar todos os filmes
    ELSE IF @codigo_lista = 4
    BEGIN;
        SELECT * FROM filme ORDER BY nome_filme ASC;
    END;
END;
